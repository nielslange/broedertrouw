...
- **Colors come from the Customizer, not CSS.** [includes/colors.php](wp-content/themes/broedertrouw/includes/colors.php) registers the brand palette via `blocksy:options:colors:palette:palettes` (same pattern as kingnature's `functions/blocksy/colors.php`); the theme's `:root` tokens in `style.css` resolve to `var(--theme-palette-color-N)` with hex fallbacks. Headline navy is palette color 4 (`#113A5C`), wired to all headings through the Customizer's "All Headings" option. Change a color in **General > Colors**, not in a stylesheet.
